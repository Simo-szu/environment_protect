...
## 2026-03-11 (low-carbon scoring bugfix + ending fallback tuning)
- Root-cause verification:
  - Confirmed `metrics.lowCarbonScore` was not reliably surfaced in client-observable state for automated diagnostics and players lacked score composition visibility.
  - Confirmed endgame mismatch pattern: very low carbon + high economy could still map to failure due strict specialization checks even when low-carbon baseline had been reached.
- Backend scoring/ending updates (`modules/game/src/main/java/com/youthloop/game/application/service/GameService.java`):
  - Added a positive-ending safety net in `applyEndingEvaluationByDocument(...)`:
    - If low-carbon baseline is reached and risk is controlled, assign a positive ending by dominant domain instead of default boundary failure.
  - Verified low-carbon breakdown plumbing remains active (`lowCarbonScoreBreakdown` in pond state), enabling target/gap observability.
- Frontend ending readability (`apps/web/src/app/[locale]/game/play/components/PlayOverlays.tsx`):
  - Replaced untranslated i18n-key render for newly-added low-carbon breakdown labels with explicit zh/en labels.
  - Ending panel now shows readable target/gap and detailed low-carbon component lines.
- Build/test verification:
  - `pnpm type-check` passed.
  - `mvn -pl modules/game -DskipTests install` passed.
  - Restarted `game-api` and confirmed Flyway at v019.
  - Playwright full-run evidence:
    - `output/web-game/full-run/20260311-162209`: reached ending with `lowCarbonScore=127`, `lowCarbonTarget=100`, `lowCarbonGap=0`.
    - `output/web-game/full-run/20260311-162751`: reached ending `ecology_priority` (non-failure), `lowCarbonScore=118`, `lowCarbonGap=0`.
  - Skill-client smoke:
    - `output/web-game/post-change-client-20260311-lowcarbon-fix`.
- Known residual signals (pre-existing, not introduced in this round):
  - Console capture still includes recurring non-blocking UI warnings (`empty string src`) and intermittent `/api/v1/system/config` 500 records in automated runs.

## Next TODO candidates
- Add targeted unit/integration tests for ending fallback branch to lock behavior (high-score baseline should not regress to boundary failure).
- Reduce console noise in automation by fixing empty-image-src warnings in ending/card image rendering paths.
- Consider exposing ending `reason` in `render_game_to_text` for easier automated reason-level assertions.

## 2026-03-11 (negative-event popup image enhancement)
- User issue: negative-event popup lacked event illustration.
- Implemented event-image rendering in popup (`PlayOverlays.tsx`):
  - Added `eventType -> imageKey` mapping:
    - `flood -> events/内涝.jpg`
    - `sea_level_rise -> events/海平面上升.jpg`
    - `citizen_protest -> events/市民抗议.jpg`
  - Added right-side image panel in each event card block with graceful fallback text when image URL unavailable.
- Validation:
  - `pnpm type-check` passed.
  - Automated run artifact with confirmed popup image:
    - `output/web-game/full-run/20260311-171705/step-042-overlay.png` (Flood popup now includes image).

## 2026-03-11 (carbon trade system stability optimization)
- Root cause analysis: Trade window was immediately closed after each transaction, preventing multiple trades per turn
- Backend optimization (`modules/game/src/main/java/com/youthloop/game/application/service/GameService.java`):
  - Modified `handleCarbonTrade()` to keep trade window open during turn for multiple transactions
  - Trade window now only closes during turn settlement or manual close operations
  - Preserved trade history and profit calculation accuracy
- Frontend enhancement (`apps/web/src/app/[locale]/game/play/hooks/useGamePlayController.ts`):
  - Enhanced error handling for trade window synchronization issues
  - Added automatic session state refresh when trade window status mismatch detected
  - Improved user feedback for trade window closure scenarios
- Translation updates:
  - Added `windowClosed` error message in both zh/en locales
  - Provides clear guidance when trade window becomes unavailable
- Verification:
  - Test run shows `tradeActions: 5` and `tradeProfit: 22.2` indicating successful multiple trades
  - No console errors during trade operations
  - Trade system now allows buy/sell operations within same turn until turn settlement
    - `Quota exceeds maximum capacity`
    - `Trade amount exceeds supported range`
  - Trade request amount now clamps to safe max before API call.
- Trade UI update (`apps/web/src/app/[locale]/game/play/components/PlayBoardAndHandsPanel.tsx`, `.../page.tsx`):
  - Wired `maxTradeAmount` into trade modal input (`max` + blur clamp).
- Localization fix for newly introduced trade validation keys:
  - `apps/web/src/i18n/locales/zh/game.ts`
  - `apps/web/src/i18n/locales/en/game.ts`

### Tests / verification
- Frontend type-check: `pnpm -C apps/web type-check` ✅
- Backend targeted tests: `mvn -pl modules/game -Dtest=GameServicePhase3Test# test` ✅ (5/5)
- Full-run regression:
  - `output/web-game/full-run/20260311-180202` (before locale key补全): consoleErrorCount=2 (missing i18n key only).
  - `output/web-game/full-run/20260311-180549`: consoleErrorCount=0, tradeActions=11, tradeProfit=21.5.
  - Visual check: `output/web-game/full-run/20260311-180549/step-final.png` inspected.

### Remaining TODO
- Add a deterministic UI script case that explicitly inputs an extreme trade amount (e.g. 999999) and asserts in-modal graceful validation text, to lock this regression in E2E level.

## 2026-03-18 (recovery replay: previous fix pack restored)
- User reported previous changes appeared lost; replayed the prior fix pack end-to-end.

### Restored backend policy uniqueness
- File: `modules/game/src/main/java/com/youthloop/game/application/service/GameService.java`
- Re-applied session-level uniqueness for policy draw candidates:
  - only policies not in hand and never drawn in current run (`countPolicyDraw == 0`) are eligible.
- Re-added regression tests:
  - `endTurnShouldNotRedrawPolicyAlreadyDrawnInCurrentSession`
  - `endTurnShouldSkipPolicyDrawWhenOnlyAlreadyDrawnPolicyExists`
  - (existing related policy draw tests retained)
- Verification:
  - `mvn -pl modules/game -Dtest=GameServicePhase3Test# test`
  - Result: ✅ 5/5 pass.

### Restored frontend core-lock popup guidance
- Files:
  - `apps/web/src/app/[locale]/game/play/hooks/useGamePlayController.ts`
  - `apps/web/src/app/[locale]/game/play/components/PlayBoardAndHandsPanel.tsx`
  - `apps/web/src/app/[locale]/game/play/page.tsx`
  ...
  - `apps/web/src/i18n/locales/en/game.ts`
- Re-applied:
  - click locked core card -> explicit modal popup (instead of silent no-op),
  - popup includes carbon/quota remaining guidance,
  - popup includes trade-window guidance (open now vs estimated turns),
  - popup includes quick action to open carbon trade when window is open.
- Verification:
  - `pnpm -C apps/web type-check` ✅ pass.
  - scripted reproduce (deploy one core then click another core):
    - `modalVisible: true`
    - artifact: `output/web-game/recover-pack-20260318/core-lock-popup.png`.

## 2026-03-18 (re-apply previous fixes after rollback)
- Re-applied top header carbon quota UX updates in `PlayHeader.tsx`:
  - restored explicit carbon quota display (`emission/quota`) and percent/status.
  - removed center turn progress strip.
  - restored compact horizontal layout style for carbon bar card (smaller footprint).
- Re-applied settlement overlay redesign in `RoundSettlementOverlay.tsx`:
  - top executive summary line.
  - key metric focus cards with trend arrows and risk emphasis.
  - event/combo module and strategy suggestion module.
  - bottom CTA text restored to `进入下一回合`.
  - right video area restored to 3:4.
- Re-applied board capacity expansion pipeline (9 -> 12 slots per domain):
  - backend `GameService` supports `boardRows/boardCols` in state and boundary checks.
  - frontend controller/selectors/panel switched to rows+cols domain split.
  - render state now exposes `board.rows` and `board.cols`.
- Re-applied settlement/header related i18n keys in `zh/en game.ts` to avoid fallback strings.

### Verification
- `pnpm -C apps/web type-check` passed.
- `mvn -pl modules/game -DskipTests compile` passed.
- Playwright smoke: `output/web-game/recover-fixes-v2-20260318/shot-0.png`
  - confirmed compact horizontal carbon bar and `12 SLOTS` per domain panel.
- Settlement screenshot: `output/web-game/full-run/20260318-123537/step-005-end_turn.png`
  - confirmed new settlement structure + bottom `进入下一回合` button.
- Note: full-run captured an existing 500 from `/api/v1/system/config` during run (`console-errors.json`), unrelated to this UI recovery patch.

## 2026-03-18 (ending archive login-gate recheck + restore)
- Restored and completed ending-page archive gating flow in web frontend:
  - `apps/web/src/app/[locale]/game/play/hooks/useGamePlayController.ts`
    - `onEndingTimeout` now short-circuits when user is not logged in.
    - `handleOpenArchive` now routes unauthenticated users to `/${locale}/login?redirect=<archiveTarget>`.
    - Added `handleLoginToSave` helper and exposed `isLoggedIn` in controller return.
  - `apps/web/src/app/[locale]/game/play/components/PlayOverlays.tsx`
    - Ending countdown copy now branches by auth state.
    - Unauthenticated ending action button now shows "登录后存档 / Login to Save" and routes to login.
  - `apps/web/src/app/[locale]/game/play/page.tsx`
    - Wired `isLoggedIn` and `handleLoginToSave` into `PlayOverlays`.
  - `apps/web/src/app/[locale]/login/page.tsx`
    - Added `redirect` query handling via `useSearchParams`.
    - After successful password/OTP/Google login, route to `safeRedirect || /${locale}`.
    - Added same-origin style guard: only accepts redirect paths starting with `/`.

- Verification executed:
  - `pnpm -C apps/web type-check` ✅
  - `pnpm game:full-run --max-steps 40 --strategy balanced` ✅ artifacts: `output/web-game/full-run/20260318-124600`
    - No gameplay regression observed in smoke flow.
    - Console captured existing `/api/v1/system/config` 500 (pre-existing env/service issue), unrelated to archive gating logic.

- Follow-up suggestion:
  - Add a small Playwright assertion specifically for ending-auth split:
    - guest: ending countdown does not navigate to archive automatically
    - logged-in: ending countdown auto-navigates to archive
    - logged-in via redirect: login success returns to `game/archive?sessionId=...`
- Additional smoke (longer): `pnpm game:full-run --max-steps 220 --strategy balanced` -> `output/web-game/full-run/20260318-124748`.
  - Run remained at settlement mode turn 2 (likely existing environment/test-runner flow issue), so automated flow did not reach ending overlay in this environment.
  - This does not affect compile-level verification of the new auth-gated ending/archive code paths.
