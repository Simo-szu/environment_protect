...
## 2026-03-11 (carbon trade high-quota stability hardening)
- User focus: system errors often occur during carbon trade, especially with high quota amounts.
- Backend hardening (`modules/game/src/main/java/com/youthloop/game/application/service/GameService.java`):
  - Added explicit high-amount guard for buy path (`amount > maxCarbonQuota` => graceful BizException).
  - Replaced unsafe `int` quota add with `long` safe-check to prevent overflow bypass (`currentQuota + amount`).
  - Added finite/range checks for computed trade value and industry delta to avoid numeric overflow path turning into system errors.
  - Added runtime sync exposure: `runtimeConfig.maxCarbonQuota` so frontend can enforce exact bound.
- Frontend hardening (`apps/web/src/app/[locale]/game/play/hooks/useGamePlayController.ts`):
  - Added computed `maxTradeAmount` (buy: min(remaining quota capacity, industry-affordable amount); sell: current quota).
  - Added pre-submit guard and user-facing blocking reasons for over-limit buy/sell.
  - Added error mapping for backend messages:
