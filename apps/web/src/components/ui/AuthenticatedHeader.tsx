...
export default function AuthenticatedHeader({ showSearch = true }: AuthenticatedHeaderProps) {
    ...
    const mobileMenuContent = (
        <>
            <div
                className="fixed inset-0 z-[90] bg-white/10 transition-opacity duration-300 md:hidden dark:bg-slate-950/10"
                onClick={() => setIsMobileMenuOpen(false)}
            />

            <div
                id="mobile-header-menu"
                className="fixed inset-x-4 z-[100] mt-0 flex max-h-[calc(100dvh-7.5rem)] flex-col overflow-y-auto rounded-[2.5rem] border border-white/20 bg-white/98 p-6 opacity-100 shadow-2xl transition-all duration-300 dark:border-slate-700/50 dark:bg-slate-900/98 md:hidden"
                style={{ top: 'calc(env(safe-area-inset-top, 0px) + 5.75rem)' }}
            >
                <button
                    type="button"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="absolute right-4 top-4 touch-manipulation p-2 text-slate-400 transition-colors hover:text-slate-600 dark:hover:text-slate-200 md:hidden"
                >
                    <X className="w-5 h-5" />
                </button>
                    ...
                    {navigationItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                href={item.href}
                                className={`
                                    group relative flex touch-manipulation items-center justify-center rounded-2xl px-6 py-4 text-base font-semibold transition-all duration-300
                                    ${isActive
                                        ? 'bg-[#56B949]/10 text-[#30499B] shadow-sm dark:text-white'
                                        : 'text-slate-500 hover:bg-slate-50 hover:text-[#30499B] dark:text-slate-400 dark:hover:bg-slate-800/50 dark:hover:text-white'}
                                `}
                                onClick={() => setIsMobileMenuOpen(false)}
                            >
                    ...
                    {(isHost || isAdmin) && (
                        <div className="flex flex-col gap-2">
                            <Link
                                ...
                                className="flex touch-manipulation items-center justify-center rounded-2xl bg-blue-50/50 px-4 py-3 text-sm font-bold text-blue-600 transition-colors hover:bg-blue-100 dark:bg-blue-900/10 dark:text-blue-400 dark:hover:bg-blue-900/20"
                                onClick={() => setIsMobileMenuOpen(false)}
                            >
                            ...
                            {isAdmin && (
                                <Link
                                    href={`/${locale}/admin`}
                                    className="flex touch-manipulation items-center justify-center rounded-2xl bg-purple-50/50 px-4 py-3 text-sm font-bold text-purple-600 transition-colors hover:bg-purple-100 dark:bg-purple-900/10 dark:text-purple-400 dark:hover:bg-purple-900/20"
                                    onClick={() => setIsMobileMenuOpen(false)}
                                >
                    ...
                    <div className="mt-2 flex items-center justify-between gap-3 px-1">
                        ...
                        {isLoggedIn ? (
                            <button
                                ...
                                className="touch-manipulation px-4 py-2 text-sm font-bold text-red-500/80 transition-colors hover:text-red-600"
                            >
                                {t('logout', 'Logout')}
                            </button>
                        ) : (
                            <div className="flex items-center gap-2">
                                <Link
                                    href={`/${locale}/login`}
                                    className="touch-manipulation text-sm font-semibold text-[#30499B] transition-colors hover:text-[#56B949]"
                                    onClick={() => setIsMobileMenuOpen(false)}
                                >
                                    {t('login', 'Login')}
                                </Link>
                                <Link
                                    ...
                                    className="touch-manipulation rounded-full bg-[#30499B] px-4 py-2 text-sm font-medium text-white shadow-md shadow-[#30499B]/20 transition-all hover:-translate-y-0.5 hover:bg-[#253a7a]"
                                    onClick={() => setIsMobileMenuOpen(false)}
                                >
                                    {t('register', 'Register')}
                                </Link>
    ...
    return (
        <nav className="relative z-[90] mt-4 rounded-[1.75rem] border border-white/70 bg-white/78 px-5 py-4 shadow-[0_24px_70px_-40px_rgba(22,101,52,0.28)] backdrop-blur-2xl ring-1 ring-[#30499B]/5 transition-all duration-500 dark:border-white/10 dark:bg-slate-900/78 dark:shadow-none dark:ring-white/10 sm:px-7">
            <div className="flex flex-wrap items-center justify-between gap-4 md:flex-nowrap md:gap-6">
                <Link href={`/${locale}`} className="group flex items-center gap-2 md:flex-shrink-0">
                    <Image
                ...
                <button
                    ...
                    className="ml-auto touch-manipulation p-2 text-[#30499B] focus:outline-none md:hidden"
                ...
                <div
                    className={`
                    hidden md:ml-auto md:flex md:w-auto md:flex-row md:border-none md:bg-transparent md:p-0 md:opacity-100 md:shadow-none
                `}
                >
                        ...
                        {navigationItems.map((item) => {
                            ...
                            return (
                                <Link
                                    ...
                                    href={item.href}
                                    className={`
                                        group relative flex touch-manipulation items-center justify-center rounded-2xl px-6 py-4 text-base font-semibold transition-all duration-300 md:justify-start md:rounded-none md:px-0 md:py-2 md:text-sm
                                        ${isActive
            ...
            {isMounted && isMobileMenuOpen ? createPortal(mobileMenuContent, document.body) : null}
        </nav>
    );
}
