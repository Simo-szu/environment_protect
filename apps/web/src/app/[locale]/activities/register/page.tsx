...
function RegisterActivityContent() {
    ...
    useEffect(() => {
        ...
        const loadActivityData = async () => {
            try {
                ...
                alert(t('form.loadActivityFailed', '加载活动信息失败'));
                router.push(`/${locale}/activities`);
            } finally {
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.name.trim()) {
            alert(t('form.nameRequired', '请填写姓名'));
            return;
        }

        if (!formData.phone.trim()) {
            alert(t('form.phoneRequired', '请填写手机号'));
            return;
        }
        ...
        if (!phoneRegex.test(formData.phone)) {
            alert(t('form.phoneInvalid', '请填写正确的手机号'));
            return;
        }

        // 如果是游客，必须填写邮箱
        if (!isLoggedIn && !formData.email.trim()) {
            alert(t('form.emailRequiredForGuest', '游客报名需要填写邮箱'));
            return;
        }

        // 如果有场次，必须选择场次
        if (sessions.length > 0 && !selectedSession) {
            alert(t('form.sessionRequired', '请选择活动场次'));
            return;
        }

        if (!formData.agreement) {
            alert(t('form.agreementRequired', '请先同意活动协议'));
            return;
        }
        ...
        try {
            ...
            alert(t('form.submitSuccess', '报名成功！'));
            ...
            alert(error.message || t('form.signupFailed', '报名失败，请重试'));
        } finally {
            setSubmitting(false);
        }
    };
            ...
            >
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            <h1 className="text-3xl font-serif font-semibold text-[#30499B]">{t('title', '活动报名')}</h1>
                            <p className="text-slate-600">{t('subtitle', '填写报名信息参与活动')}</p>
                        </div>
                    </div>
                </div>
            </motion.div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    >
                        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-white/60 shadow-lg sticky top-8">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="font-semibold text-[#30499B]">{t('activityInfo.title', '活动信息')}</h3>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    {activity.maxParticipants && (
                                        <div className="flex items-center gap-2 text-slate-600">
                                            ...
                                            <span>{activity.currentParticipants}/{activity.maxParticipants} {t('activityInfo.participants', '人')}</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                {activity.organizerName && (
                                    <div className="pt-4 border-t border-slate-200">
                                        <p className="text-xs text-slate-500 mb-2">{t('activityInfo.organizer', '主办方')}</p>
                                ...
                                {sessions.length > 0 && (
                                    <div className="pt-4 border-t border-slate-200">
                                        <p className="text-xs text-slate-500 mb-2">{t('form.selectSession', '选择场次')} *</p>
                    ...
                    >
                        <div className="bg-white/80 backdrop-blur-sm rounded-xl p-8 border border-white/60 shadow-lg">
                            <h3 className="text-xl font-semibold text-[#30499B] mb-6">{t('form.title', '报名信息')}</h3>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder={t('form.namePlaceholder', '请输入真实姓名')}
                                            required
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={handleInputChange}
                                            className="w-full px-4 py-3 border border-slate-200 rounded-lg focus:border-[#56B949] focus:outline-none transition-colors"
                                            placeholder={t('form.phonePlaceholder', '请输入手机号码')}
                                            required
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-2">
                                        {t('form.email', '邮箱地址')} {!isLoggedIn && <span className="text-red-500">*</span>}
                                    </label>
                                    <input
                                        type="email"
                                        name="email"
                                        ...
                                        onChange={handleInputChange}
                                        className="w-full px-4 py-3 border border-slate-200 rounded-lg focus:border-[#56B949] focus:outline-none transition-colors"
                                        placeholder={isLoggedIn ? t('form.emailOptional', '选填') : t('form.emailRequired', '游客报名必填邮箱')}
                                        required={!isLoggedIn}
                                    />
                                    {!isLoggedIn && (
                                        <p className="text-xs text-slate-500 mt-1">{t('form.emailNote', '游客报名需要提供邮箱以接收活动通知')}</p>
                                    )}
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-2">
                                        {t('form.specialRequirements', '备注信息')}
                                    </label>
                                    <textarea
                                        name="remarks"
                                        ...
                                        placeholder={t('form.specialRequirementsPlaceholder', '如有特殊需求或其他需要说明的情况，请在此填写...')}
                                    />
                                </div>
                                ...
                                <div className="flex items-start gap-3 p-4 bg-[#56B949]/5 rounded-lg border border-[#56B949]/20">
                                    ...
                                    <div className="text-sm">
                                        <p className="text-slate-700">
                                            {t('form.agreementText', '我已阅读并同意活动相关协议，了解活动风险，自愿参与此次环保活动。')}
                                        </p>
                                        <p className="text-slate-500 mt-1">
                                            {t('form.agreementNote', '参与活动期间请遵守组织方安排，注意人身安全。')}
                                        </p>
                                    </div>
                                </div>
